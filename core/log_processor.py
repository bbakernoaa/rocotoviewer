...
class StreamingLogProcessor:
    ...
    def register_stream_callback(
        self, log_path: Path, callback: Callable[[Dict[str, Any]], None]
    ):
        """
    ...
    def unregister_stream_callback(
        self, log_path: Path, callback: Callable[[Dict[str, Any]], None]
    ):
        """
    ...
    def process_new_log_line(
        self, log_path: Path, line: str
            ...
            if log_path in self.stream_callbacks:
                for callback in self.stream_callbacks[log_path]:
                    try:
                        ...
                        self.logger.error(
                            f"Error in stream callback for {log_path}: {str(e)}"
                        )

        return parsed_entry

    def read_log_file_streaming(
        self, log_path: Path, max_lines: Optional[int] = None
    ...
    def parse_log_line(self, line: str) -> Dict[str, Any]:
        ...
        if timestamp_match:
            try:
                ...
                try:
                    ...
                    result["timestamp"] = datetime.strptime(
                        timestamp_match.group(), "%Y-%m-%d %H:%M:%S.%f"
                    )
                except ValueError:
        ...
        if task_match:
            result["task_id"] = (
                task_match.group(1)
                if len(task_match.groups()) > 0
                else task_match.group()
            )
        ...
        if result["level"] in message:
            ...
            message = re.sub(
                r"^" + re.escape(result["level"]), "", message, count=1
            ).strip()
    ...
    def filter_logs(
        ...
        if search_term:
            ...
            filtered_logs = [
                ...
                or (log["task_id"] and search_lower in log["task_id"].lower())
            ]

        if start_time:
            filtered_logs = [
                log for log in filtered_logs if log.get("received_at", 0) >= start_time
            ]
    ...
    def get_recent_logs(self, log_path: Path, count: int = 100) -> List[Dict[str, Any]]:
        ...
        if log_path in self.stream_buffers:
            ...
            recent_entries = (
                list(buffer)[-count:] if len(buffer) > count else list(buffer)
            )
    ...
    def get_streaming_logs(
        self, log_path: Path, count: int = 100
    ) -> List[Dict[str, Any]]:
        """
...
class LogProcessor:
    ...
    def filter_logs(
        ...
        Returns:
            Filtered list of log dictionaries
        """
        return self.streaming_processor.filter_logs(
            logs, level, task_id, status, search_term, start_time, end_time
        )
